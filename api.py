...
def login_page():
    token = session.get('email') is not None
    return render_template('login.html', user_is_authenticated=token, opening_hours=get_opening_hours())
...
def login():
    if request.method == 'POST':
        ...
        print(f'Mot de passe hacher: {stored_hash}')  # Debug: affiche le mot de passe hacher

        if stored_hash is not None:
            if check_password(entered_password, stored_hash):
                # Les mdp correspondent, l'utilisateur est donc authentifié
                session['email'] = email
                session['role'] = role  # Attribut le role a la session
                print('Connexion réussie')
                return jsonify({"success": True})
            else:
                # Les mdp ne correspondent pas la connexion est rejeter
